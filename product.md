...
| ID | 功能 | 状态 | 说明 |
|----|------|------|------|
| F26 | 关注列表修复 | ✅ 已完成 | 修复后端 `user_id` 非标准 ObjectID 解析失败问题，使用 MD5 哈希生成固定 ObjectID |
| F27 | 监控账号数据模型 | ✅ 已完成 | 后端 `MonitoredAccount` 数据模型（MongoDB）、仓库层 CRUD 操作 |
| F28 | 监控账号 API | ✅ 已完成 | 后端 4 个端点：获取列表、添加监控、取消监控、批量检查状态 |
| F29 | 关注列表监控按钮 | ✅ 已完成 | 前端关注列表每行添加「添加监控/取消监控」按钮，带眼睛图标和状态标签 |
| F30 | 监控账号列表页 | ✅ 已完成 | 前端「监控 → 监控账号」页面，展示已监控账号，支持取消监控 |
| F31 | 推文数据模型 | ✅ 已完成 | 后端 `Tweet`/`TweetComment` 数据模型，支持推文和评论存储 |
| F32 | 推文动态 API | ✅ 已完成 | 后端 3 个端点：获取推文列表、刷新推文、获取推文评论 |
| F33 | 推文动态页面 | ✅ 已完成 | 前端「监控 → 推文动态」页面，展示监控账号推文、互动数据（点赞/转发/评论/浏览） |
| F34 | 评论展示（Top 10） | ✅ 已完成 | 推文可展开评论区，展示前 10 条评论（按点赞排序），含评论者信息、内容、点赞数 |
| F35 | bird-cli AUTH_TOKEN 支持 | ✅ 已完成 | 后端 bird client 优先使用环境变量 `AUTH_TOKEN`/`CT0`，无需 Chrome 登录 |
| F36 | 评论获取修复 | ✅ 已完成 | 修复推文动态下看不到评论的问题：统一使用 Chrome profile 获取评论（避免 AUTH_TOKEN/CT0 证书问题），支持通过 `BIRD_USE_TOKEN=true` 环境变量强制使用 token |
| F37 | 头像与蓝V展示 | ✅ 已完成 | 推文动态和评论列表展示用户头像；推文作者和评论作者显示蓝V认证标识（Twitter Blue）|
...
**监控账号相关：**
- `GET /api/monitored-accounts?user_id=xxx` - 获取监控账号列表
- `POST /api/monitored-accounts` - 添加监控账号
- `DELETE /api/monitored-accounts?user_id=xxx&username=xxx` - 取消监控
- `POST /api/monitored-accounts/check` - 批量检查监控状态

**推文动态相关：**
- `GET /api/tweets?user_id=xxx&limit=50` - 获取监控账号推文列表
- `POST /api/tweets/refresh` - 刷新所有监控账号推文（异步）
- `GET /api/tweets/comments?platform_post_id=xxx&limit=10` - 获取推文前10条评论
